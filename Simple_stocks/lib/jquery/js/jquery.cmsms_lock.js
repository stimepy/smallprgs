...
(function($){
  $.widget('cmsms.lockManager', {
	...
	if( self._settings.locked && self._settings.lock_id > 0 ) {
		...
		console.debug('sending a beacon');
		navigator.sendBeacon(self._settings.ajax_url,JSON.stringify(opts));
		...
		var _jqxhr = $.ajax({
		    ...
		    console.debug('in pipe '+textStatus+' status is '+xhr.status);
		    if( xhr.status != 200 ) {
			console.debug('pipe status is '+xhr.status);
			return $.Deferred().reject(data,textStatus);
		    }
		    if( textStatus != 'success' ) {
			console.debug('pipe bad textStatus');
			return $.Deferred().reject(data,textStatus);
		    }
  		    if( data.status == 'error' ) {
			console.debug('pipe got status = error');
			return $.Deferred().reject(data,textStatus);
		    }
